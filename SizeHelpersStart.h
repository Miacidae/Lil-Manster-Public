...
.if (!GUARD_SIZE_HELPERS_START)
  ...
  ; Macros
    ...
    startGeneralText .segment Pointer=*
      GeneralTextStarts ..= [\Pointer]
    .endsegment

    endGeneralText .segment Pointer=*
  ...
  ; Helpers
    ...
    SizeFormatter .sfunction dSize, ROMSize=$600000, format("$%06X %%%05.2f", dSize, 100.0-((float(ROMSize-dSize)/float(ROMSize)*100)))
